...
vietnam_regions$name <- c("Nord", "NCC", "CHR", "Sud-est", "MDR", "Delta du Fleuve Rouge")
...
vnts |>
  ...
  theme_minimal() +
  labs(x = "température (deg. Celsius)", y = "densité")

vnts |>
  mutate(t_max = as.fd(center(t_max))) |>
  plot_funs(t_max, color = region) +
  theme_minimal() +
  labs(x = "température (deg. Celsius)", y = "centered clr(density)")
```
...
vnt |>
  mutate(
    t_max = as.fd(center(t_max)),
    province = ifelse(outlying == 1, province, "non atypique")
  ) |>
  plot_funs(t_max, color = province, alpha = outlying) +
  facet_wrap(vars(year)) +
  labs(x = "température (deg. Celsius)", y = "centered clr(density)")
...
icsout$result |>
  ...
  labs(x = "température (deg. Celsius)", y = "clr(densité)")
```
