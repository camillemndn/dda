...
```{r pairwise}
# vntp <- crossing(region1 = vietnam_regions$code, region2= vietnam_regions$code) |>
#   rowwise() |>
#   mutate(
#     data = map2(region1, region2, function(region1, region2) vnte[vnt_province_region[colnames(vnte)] %in% c(region1, region2)]),
# res = lapply(data, function(data) fanova.tests(data, vnt_province_region[colnames(data)],
#   test = stat_list,
#   ) |> pull(as_vector(stat_list)) |>
#   sapply(function(x) c(`Statistic` = x$stat, `p-value` = x$pval)) |>
#   t() |>
#   as.data.frame() |>
#   rownames_to_column("Name") |>
