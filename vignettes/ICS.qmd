...
#|  - Centered log-ratio transform of the centered densities of maximum temperature over 28 provinces of Vietnam and during the years 2013-2016, color-coded by region.
#|  - Centered log-ratio transform of the densities of maximum temperature over 28 provinces of Vietnam and during the years 2013-2016. The case of LAICHAU.
#| fig-width: 7.5
#| fig-height: 5
#| layout-ncol: 2
...
#| fig-cap: "Outlier detection using ICS on 28 provinces of Vietnam, during the years 2013-2016, over a grid of preprocessing parameters. Top: setting knots at quantiles; bottom: using equally spaced knots. Columns correspond to the number of inside knots (0-35). Outliers are marked as black squares."

ics_grid_out |>
  filter(nknots %% 2 == 0 | nknots > 20) |>
  ggplot(aes(log10(lambda), obs_id, fill = outlying)) +
  geom_tile() +
...
#| tbl-cap: Outlier detection using ICS on 28 provinces of Vietnam, during the years 2013-2016, over a grid of preprocessing parameters.
...
#| fig-cap: Outlier detection using ICS on 28 provinces of Vietnam, during the years 2013-2016, over a grid of preprocessing parameters.

ics_grid_summary |>
  ggplot(aes(obs_id, n_outlying)) +
  geom_bar(stat = "identity") +
  labs(x = "observation index", y = "outlier count")
...
#| fig-cap: Densities of maximum temperatures in the 13 provinces of the S3 climate region in Vietnam over 30 years.
...
#| fig-cap: ICS on densities of maximum temperature over 13 provinces of Vietnam and during 30 years.
#| fig-subcap:
...
icsout$result |>
  plot_funs(H_dual, color = index, alpha = selected) +
...
#| fig-cap:
#|  - Densities of maximum temperature of the provinces in the S3 climate region of Vietnam during the years 1987-2016.
#|  - Centered log-ratio transforms of the centered densities of maximum temperature of the provinces in the S3 climate region of Vietnam during the years 1987-2016.
#| fig-width: 7.5
#| fig-height: 5
#| layout-ncol: 2
...
#| fig-cap: ICS distances of the densities of maximum temperature of the provinces in the S3 climate region of Vietnam during the years 1987-2016.
#| fig-width: 7.5
#| fig-height: 5

vnt |>
  mutate(province = ifelse(outlying == 1, province, "not outlying")) |>
  ggplot(aes(index, distances, color = province, label = year)) +
  geom_text() +
  geom_hline(yintercept = icsout$ics_dist_cutoff)
```

```{r fig-climate_regions_scatter}
#| fig-cap: Selected invariant coordinates of the densities of maximum temperature of the provinces in the S3 climate region of Vietnam during the years 1987-2016.
#| fig-width: 7.5
#| fig-height: 5
...
ics_grid_summary |>
  ...
  geom_bar(stat = "identity") +
  labs(y = "outlier count")
```

## Constraining the centered-log ratio transforms to 0 on the boundaries
...
#| fig-cap: Outlier detection using ICS on the S3 climate region of Vietnam, with centered-log ratio transforms constrained to 0 on the boundaries.
...
```{r fig-ten_periods_out}
#| fig-cap:
#|  - Select invariant components of the densities of maximum temperature in the two northern regions of Vietnam averaged over three-year periods.
...
vnt |> plot_matrix(paste0("IC.", 1:4),
  ...
  alpha = outlying == 1
) + labs(color = "province", x = "", y = "") + guides(alpha = "none")
