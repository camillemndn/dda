...
simulation_gp_clr <- function(n, p = 50) {
  sim <- fdaoutlier::simulation_model4(n, p, mu = 15)
  y <- exp(tfd(sim$data))
...
tibble(Scheme = schemes) |>
  ...
  mutate(sim = map(sim, function(x) {
    tibble(
      data = tfd(x$data),
      `true outlier` = ifelse(x$true_outliers == 1, TRUE, NA)
    )
  })) |>
  unnest(sim) |>
  ggplot() +
  geom_spaghetti(aes(y = data, color = `true outlier`)) +
...
methods <- c("ICS", "LQD", "Wasserstein", "MBD")
N <- 50 # Number of repetitions per combination

if (!(file.exists("ICS_comparison.RData"))) {
  method_ics <- function(data) {
    f <- fda.usc::fdata(data) |> fda.usc::fdata2fd()
    ICS_outlier(f, n_cores = 14)$ics_distances
  }

  method_deboinr <- function(data, distance) {
    density_order <- deboinr(seq(0, 1, length.out = ncol(data)), data, distance = distance, num_cores = 14)$density_order
    n <- nrow(data)
