...
vietnam_temperature_dd <- vietnam_temperature |>
  ...
  mutate(period = if_else(year >= 2000, "2011-2016", "1987-1992")) |>
...
selected_years <- c(1987:1992, 2011:2016)
...
vnt <- vnt |>
  ...
  mutate(evolution = `2011-2016` - `1987-1992`) |>
  pivot_longer(cols = c("2011-2016", "1987-1992", "evolution"), names_to = "period", values_to = "t_max")

vnt |> plot_funs(t_max, color = region_name) +
  facet_grid(vars(period), vars(str_wrap(region_name, 20))) +
  ylim(0, 0.85) +
...
vnt |> plot_funs(t_max, color = region_name) +
  ...
  ylim(0, 0.85) + theme_legend_inside +
  labs(
    x = "Temperature (deg. Celsius)",
    y = "Density",
    color = "Region"
...
#| fig-cap:
#|  - Mean negative perturbation of maximum temperature distribution between 1987-1992 and 2011-2016.
#|  - Centered log-ratio transform of the mean negative perturbation of maximum temperature distribution between 1987-1992 and 2011-2016.
...
#| fig-cap: Correlation function within the MDR region (2011-2016).

vnt_per <- vnt |> filter(period == "2011-2016" & region == "MDR")
...
```{r fig-danova_global}
#| fig-cap: "Null distributions of the global DANOVA tests (black: bootstrap, red: Welch–Satterthwaite approximation)."
...
if (file.exists("DANOVA_6.RData")) {
  ...
  save(f_max_dist, file = "DANOVA_6.RData")
}
...
save(res, file = "DANOVA_6.RData")
```


# One-sample tests on the regional mean negative perturbations between 1987-1992 and 2011-2016
